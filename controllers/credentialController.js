    ...
    listall: async (req, res) => {
        try {
            ...
            const [totalCount, result] = await Promise.all([
                prisma.credential.count({ where: whereCondition }),
            ...
            // Extract all clientIds for lookup
            const clientIds = result.map(r => r.customerTenantId);

            console.log("clientIds", clientIds)

            // Fetch customers where acronisCustomerTenantId IN clientIds
            const customerList = await prisma.customer.findMany({
                where: {
                    acronisCustomerTenantId: { in: clientIds }
                },
                select: {
            ...
            customerList.forEach(c => {
                customerMap[c.acronisCustomerTenantId] = c.acronisCustomerTenantName;
            });
            ...
            const enrichedResult = result.map(item => ({
                ...item,
                customerName: customerMap[item.customerTenantId] || "-"
