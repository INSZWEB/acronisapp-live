...
// CONTACT MAPPER
// -------------------------------------------
function mapContactToDb(contact, partnerTenantId, acronisCustomerTenantId) {
  return {
    apiId: contact.id,

    apiCreatedAt: contact.created_at
      ? new Date(contact.created_at)
      : null,
    apiUpdatedAt: contact.updated_at
    ...
    partnerTenantId,
    customerTenantId: acronisCustomerTenantId,
...
// MAIN SERVICE
// -------------------------------------------
async function syncContacts(partnerTenantId, acronisCustomerTenantId) {
  // 1️⃣ Get active partner credential
  ...
  if (!credential) {
    throw new Error(
      `No active credential found for partnerTenantId: ${partnerTenantId}`
    );
  }
  ...
  // 3️⃣ Fetch contact IDs from customer tenant
  const contactsResponse = await apiGet(
    `${credential.datacenterUrl}/api/2/tenants/${acronisCustomerTenantId}/contacts`,
    token
  );
  ...
  // 4️⃣ Fetch contact details & upsert
  let count = 0;
  ...
  for (const contactId of contactsResponse.items) {
    ...
    const dbData = mapContactToDb(
      contactDetails,
      partnerTenantId,
      acronisCustomerTenantId
    );

    await prisma.customerAcronisContact.upsert({
